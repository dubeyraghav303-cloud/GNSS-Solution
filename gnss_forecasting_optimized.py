"""
Optimized GNSS Time-Series Forecasting Solution
==============================================

A faster version with reduced TensorFlow overhead and better performance.
"""
...
def lazy_import_tensorflow():
    ...
    try:
        ...
        return tf, keras, layers, Model, optimizers, callbacks, Input, Dense, LSTM, GRU, Dropout, LayerNormalization, MultiHeadAttention, GlobalAveragePooling1D, Adam, EarlyStopping, ReduceLROnPlateau, ModelCheckpoint
...
class OptimizedGNSSDataProcessor:
    """Optimized data processor with better performance."""
    ...
    def load_data(self) -> pd.DataFrame:
        """Load and combine all GNSS data files efficiently."""
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Optimized preprocessing with better performance."""
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Lag features for error variables (reduced for speed)
        error_columns = ['x_error (m)', 'y_error (m)', 'z_error (m)', 'satclockerror (m)']
        lag_steps = [1, 4]  # Reduced from [1, 4, 8] for speed
        
        for col in error_columns:
            for lag in lag_steps:
                df[f'{col}_lag_{lag}'] = df[col].shift(lag)
        
        # Rolling statistics (reduced windows for speed)
        windows = [4]  # Reduced from [4, 16] for speed
    ...
    def prepare_training_data(self, df: pd.DataFrame, sequence_length: int = 48) -> Tuple[np.ndarray, np.ndarray]:
        """Optimized training data preparation with reduced sequence length."""
        ...
        # Create sequences with reduced output length for speed
        X, y = [], []
        output_length = 24  # Reduced from 96 to 24 (6 hours instead of 24 hours)
        
        for i in range(sequence_length, len(scaled_features) - output_length):
            X.append(scaled_features[i-sequence_length:i])
            y.append(scaled_features[i:i+output_length, :4])  # Next 6 hours of error predictions
...
class OptimizedTransformerLSTMModel:
    """Optimized hybrid Transformer-LSTM model with reduced complexity."""
    
    def __init__(self, input_shape: Tuple[int, int], output_length: int = 24):
        self.input_shape = input_shape
        self.output_length = output_length
        self.model = None
        self.tf_components = None
        
    def build_model(self):
        """Build optimized hybrid Transformer-LSTM model."""
        print("Building optimized Transformer-LSTM model...")
        
        # Lazy import TensorFlow components
        if self.tf_components is None:
            self.tf_components = lazy_import_tensorflow()
            if self.tf_components is None:
                raise ImportError("TensorFlow not available")
        
        (tf, keras, layers, Model, optimizers, callbacks, Input, Dense, LSTM, GRU, 
         Dropout, LayerNormalization, MultiHeadAttention, GlobalAveragePooling1D, 
         Adam, EarlyStopping, ReduceLROnPlateau, ModelCheckpoint) = self.tf_components
        
        # Input layer
        inputs = Input(shape=self.input_shape, name='input_sequence')
        
        # Simplified Transformer Encoder (reduced complexity)
        ...
        ffn = Dense(128, activation='relu')(attention_output)  # Reduced from 256
        ffn = Dropout(0.1)(ffn)
        ...
        lstm1 = LSTM(64, return_sequences=True, dropout=0.2)(transformer_output)  # Reduced from 128
        lstm2 = LSTM(32, return_sequences=False, dropout=0.2)(lstm1)  # Reduced from 64
        
        # Simplified dense layers
        dense1 = Dense(64, activation='relu')(lstm2)  # Reduced from 128
        dense1 = Dropout(0.3)(dense1)
        dense2 = Dense(32, activation='relu')(dense1)  # Reduced from 64
        ...
        # Create model
        model = Model(inputs=inputs, outputs=outputs, name='OptimizedTransformerLSTM_GNSS')
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        kl_loss = 0.5 * (
            ...
            tf.square(pred_mean) - 
            1.0 + 
            2 * tf.math.log(pred_std + 1e-8)
        )
        
        # Combine losses with reduced weight
        total_loss = mse_loss + 0.05 * tf.reduce_mean(kl_loss)  # Reduced from 0.1
    ...
    def train_model(self, X_train, y_train, X_val, y_val, epochs: int = 20, batch_size: int = 32):
        """Train the optimized model with reduced epochs."""
        ...
        (EarlyStopping, ReduceLROnPlateau, ModelCheckpoint) = self.tf_components[16:19]
...
class OptimizedGNSSForecaster:
    """Optimized main class for GNSS time-series forecasting."""
    
    def __init__(self, data_dir: str = "."):
        self.data_processor = OptimizedGNSSDataProcessor(data_dir)
        self.model = None
        self.history = None
    ...
    def train_model(self, sequence_length: int = 48, epochs: int = 20):
        ...
        self.model = OptimizedTransformerLSTMModel(
            input_shape=(sequence_length, X.shape[-1]),
            output_length=24  # 6 hours instead of 24 hours
        )
    ...
    def predict_sixth_day(self) -> np.ndarray:
        """Generate predictions for the next 6 hours."""
        print("Generating predictions for next 6 hours...")
        
        # Use the last sequence from training data as input
        X, _ = self.data_processor.prepare_training_data(self.featured_data, 48)
        ...
        # Reshape predictions
        predictions = predictions.reshape(-1, 4)  # 24 time steps, 4 error variables
    ...
    def evaluate_predictions(self, predictions: np.ndarray, actual: np.ndarray = None) -> Dict[str, float]:
        ...
        horizons = [1, 2, 4, 8, 16, 24]  # 15min, 30min, 1hr, 2hr, 4hr, 6hr
        rmse_results = {}
    ...
    def plot_results(self, predictions: np.ndarray, evaluation_results: Dict):
        ...
        axes[0, 0].set_title('GNSS Error Predictions (6 Hours)')
        ...
        axes[1, 1].hist(synthetic_errors, bins=20, alpha=0.7, color='green', density=True)
        ...
        plt.tight_layout()
        plt.savefig('optimized_gnss_results.png', dpi=200, bbox_inches='tight')
        plt.show()
...
def main():
    """Main execution function for optimized solution."""
    print("=" * 60)
    print("GNSS Time-Series Forecasting Solution (Optimized)")
    print("=" * 60)
    
    # Initialize forecaster
    forecaster = OptimizedGNSSForecaster()
    ...
    print("-" * 40)
    history = forecaster.train_model(epochs=15)  # Reduced epochs for speed
    
    # Step 3: Generate predictions
    print("\n3. Generating Predictions")
    print("-" * 40)
    predictions = forecaster.predict_sixth_day()
    ...
    print("\n" + "=" * 60)
    print("Optimized GNSS Forecasting Solution Complete!")
    print("=" * 60)
